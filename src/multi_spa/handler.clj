(ns multi-spa.handler
  (:require [compojure.core :refer [defroutes routes wrap-routes]]
            [multi-spa.routes.home :refer [home-routes]]
            
            [multi-spa.middleware :as middleware]
            [multi-spa.session :as session]
...
(defn init
  ...
  (timbre/set-config!
    ...
    {:path "multi_spa.log" :max-size (* 512 1024) :backlog 10})

  (if (env :dev) (parser/cache-off!))
  (start-nrepl)
  ;;start the expired session cleanup job
  (session/start-cleanup-job!)
  (timbre/info "\n-=[ multi-spa started successfully"
...
(defn destroy
  ...
  []
  (timbre/info "multi-spa is shutting down...")
  (stop-nrepl)
